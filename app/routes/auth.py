...
def add_address():
    if request.method == 'POST':
        ...
        if not all([recipient_name, phone, address_detail, commune, province]):
            flash('Vui lòng điền đầy đủ thông tin!', 'danger')
            return redirect(url_for('auth.add_address'))
        
        # Construct full address
        address = f"{address_detail}, {commune}, {province}"
...
def edit_address(address_id):
    address_obj = Address.query.get_or_404(address_id)
    
    # Check ownership
    if address_obj.user_id != current_user.id:
    ...
    if request.method == 'POST':
        ...
        if not all([recipient_name, phone, address_detail, commune, province]):
            flash('Vui lòng điền đầy đủ thông tin!', 'danger')
            return redirect(url_for('auth.edit_address', address_id=address_id))
        
        # Construct full address
        full_address = f"{address_detail}, {commune}, {province}"
        
        # Update address object
        address_obj.label = label if label else None
        address_obj.recipient_name = recipient_name
        address_obj.phone = phone
        address_obj.address = full_address
        
        if is_default and not address_obj.is_default:
            # Remove default from other addresses
            Address.query.filter_by(user_id=current_user.id, is_default=True).update({'is_default': False})
            address_obj.is_default = True
        elif not is_default and address_obj.is_default:
    ...
    return render_template('edit_address.html', address=address_obj)
