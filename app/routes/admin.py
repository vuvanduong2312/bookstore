...
from app import db
from app.models import Book, Category, Author, Order, OrderDetail, User, Address, PaymentConfig
from app.utils import BANK_CODES
...
@bp.route('/payment-config', methods=['GET', 'POST'])
@login_required
@admin_required
def payment_config():
    if request.method == 'POST':
        ...
        config.bank_name = request.form.get('bank_name')
        config.bank_code = request.form.get('bank_code')
        config.account_number = request.form.get('account_number')
        config.account_name = request.form.get('account_name')
    ...
    config = PaymentConfig.query.first()
    return render_template('admin/payment_config.html', config=config, bank_codes=BANK_CODES)
