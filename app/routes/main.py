...
from app import db
from app.models import Book, Category, Author, Order, OrderDetail, CartItem, Address, PaymentConfig
from app.utils import generate_payment_qr
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        if address_id:
            # Use saved address
            saved_address = Address.query.get_or_404(address_id)
            if saved_address.user_id != current_user.id:
            ...
            # Use manually entered information
    ...
    # Get payment config and generate QR preview
    payment_config = PaymentConfig.query.filter_by(is_active=True).first()
    qr_preview_url = None
    if payment_config and total > 0:
        qr_preview_url = generate_payment_qr(
            bank_code=payment_config.bank_code,
...
@bp.route('/order/<int:order_id>/payment')
@login_required
def order_payment(order_id):
    ...
    qr_url = None
    if payment_config and order.payment_method != 'COD':
        # Generate QR code for this order
        qr_url = generate_payment_qr(
            bank_code=payment_config.bank_code,
            account_number=payment_config.account_number,
            account_name=payment_config.account_name,
        ...
        # Save QR URL to order
        if not order.payment_qr_code:
            order.payment_qr_code = qr_url
            db.session.commit()
