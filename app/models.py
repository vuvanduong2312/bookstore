...
class Order(db.Model):
    ...
    payment_qr_code = db.Column(db.String(500))  # URL QR code thanh toán
    payment_status = db.Column(db.String(20), default='Chưa thanh toán')  # Chưa thanh toán, Đã thanh toán
...
class PaymentConfig(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    bank_name = db.Column(db.String(100), nullable=False)  # Tên ngân hàng
    bank_code = db.Column(db.String(20), nullable=False)   # Mã ngân hàng (VCB, TCB, MB...)
    account_number = db.Column(db.String(50), nullable=False)  # Số tài khoản
    account_name = db.Column(db.String(100), nullable=False)   # Tên chủ tài khoản
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    def __repr__(self):
        return f'<PaymentConfig {self.bank_name} - {self.account_number}>'
